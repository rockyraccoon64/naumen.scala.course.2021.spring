...
object Exercises {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        ...
        numbers.foldLeft(0 : Long)((x,y) => x + y)
    }
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        var mutableList = ListBuffer.empty[Int]
        while (currentNumber > 1 && currentDivisor <= currentNumber) {
            if (currentNumber % currentDivisor == 0) {
                mutableList += currentDivisor
    ...
    */
    ...
    def getMass(ball: (String, (Int, Double))) : Double = {
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq
          .sortWith((ball1, ball2) => { getMass(ball1) < getMass(ball2) })
          .map(ball => ball._1)
    }
